...
const ProductList = () => {
    return (
        <>
            {DUMMY_PRODUCT_DATA.map((product) => (
             
        <div class="col-12 col-sm-4 col-md-3 col-lg-2 px-2 ">
                <div class={classes.product_card_inner}>
                    <div class={classes.product_image_box}>
                        <img src={product.image_src[0]} className='img-fluid' alt="lgo" />
                    </div>
                    <div class={`${classes.mini_description_box} px-2`}>
                        <p class={`${classes.product_brand_name} m-0`}>{product.vendor}</p>
                        <p class={`${classes.product_description} m-0`}>{product.name}</p>
                        <p class={`${classes.product_price} m-0`}>${product.price} 
                            <strike class={classes.product_actual_price}>${product.compare_at_price} </strike> 
                            <span className={classes.product_price_discount}>(50% OFF)</span> 
                        </p>
                    </div>
                </div>
            </div>
